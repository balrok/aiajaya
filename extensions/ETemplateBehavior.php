...
class ETemplateBehavior extends CActiveRecordBehavior
{
	public $contentVar;
	...
	public function getSavePath($depth=0)
	{
		...
		$path = $this->basePath;
		$path[] = $this->getOwner()->getPrimaryKey();
		if ($depth == -1)
			array_pop($path);
		return implode('.', $path);
	}
	...
	private function _saveToFile()
	{
		...
		$html = $this->getReplacedContent($this->getOwner()->{$this->contentVar}, $this->getReplaceArray());
...
EOP;
	...
	protected static function escapeForRegex($str)
	{
		$patterns = array('/\//', '/\^/', '/\./', '/\$/', '/\|/',
				'/\(/', '/\)/', '/\[/', '/\]/', '/\*/', '/\+/',
				'/\?/', '/\{/', '/\}/', '/\,/');
	...
	protected function getReplacedContent($content, $replArray)
	{
		...
		{
			$search = (strpos($search, '/') === 0) ? $search : '/'.self::escapeForRegex($search).'/';
	...
	*/
	...
	public function getOneHelper($models, $replace)
	{
		...
		if ($models[0] == 'this' && strpos($replace,';')===false)
		{
			if (($pos = strpos($replace, '(')) !== false)
				$val = call_user_func_array(array($this->getOwner(), substr($replace, 0, $pos)), explode(',', substr($replace,$pos+1,-1)));
			else
				$val = $this->getOwner()->$replace;
	...
	public function afterSave($event)
	{
		$this->_removeFile();
		if (!$this->_saveToFile())
			$this->getOwner()->addError('file', Yii::t('extension', 'Fehler beim Speichern der Datei'));
	}
