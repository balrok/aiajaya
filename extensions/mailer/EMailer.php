 ...
 *
 * @author MetaYii
 * @package aiajaya.extensions.emailer 
 * @since 1.0
 */
class EMailer
{
	// wont send the mail for real
	public $yiiDebug = true;
	// default target - will send the email only to a specific mail.. can be usefull when debugging :)
	public $yiiDefaultMail = '';
    ...
    * @var string
    */
   protected $pathViews = 'aiajaya.views.email';
    ...
    * @var string
    */
   protected $pathLayouts = 'aiajaya.views.email.layouts';
    ...
    */
	...
	{
		$this->_myMailer = new PHPMailer(false);
	}
   ...
   public function setPathLayouts($value)
   {
	...
	public function validateAddress($addr)
   	{
		return PHPMailer::ValidateAddress($addr);
   	}
   ...
   public function getView($view, $vars = array(), $layout = null)
   {
	...
	// this wraps around the mailer to enable yii-debugging
   public function Send()
   {
	   if (!empty($this->yiiDefaultMail))
		   $this->To = $this->yiiDefaultMail;

	   $success = true;
	   if (!$this->yiiDebug)
	   {
		   $mailer = $this->_myMailer;
